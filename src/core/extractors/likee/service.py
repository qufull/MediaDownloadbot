...
class LikeeExtractor:
    """
    Экстрактор медиа-контента из Likee через yt-dlp.

    Поддерживает:
    - Короткие видео (likee.video, l.likee.video)
    ...
    # Паттерны для распознавания ссылок Likee
    URL_PATTERNS = [
        r"likee\.video",
        r"l\.likee\.video",
        r"likee\.com",
    ...
    def get_error_description(self, code: Any) -> str:
        descriptions = {
            "LIKEE_INVALID_URL": "Неверный или неподдерживаемый URL Likee",
            "LIKEE_EXTRACT_ERROR": "Не удалось извлечь медиа из Likee",
            "LIKEE_NO_MEDIA": "Медиа не найдено в публикации Likee",
            "LIKEE_PRIVATE_CONTENT": "Контент Likee недоступен (возможно, приватный)",
            "LIKEE_UNEXPECTED_ERROR": "Непредвиденная ошибка при обработке Likee",
        }
        return descriptions.get(_safe_str(code), _safe_str(code, default="LIKEE_EXTRACT_ERROR"))
    ...
    def _validate_url(url: str) -> bool:
        try:
            ...
            return any(
                ...
                for d in LikeeExtractor.SUPPORTED_DOMAINS
            )
        except Exception:
            return False
    ...
    def _extract(self, url: str) -> Dict[str, Any]:
        if not self._validate_url(url):
            return {
                "status": "error",
                "code": "LIKEE_INVALID_URL",
                "context": f"URL не является поддерживаемым URL Likee: {url}",
                "data": None,
            }
        ...
        try:
            ...
            logger.info(
                "Likee extracted: %d videos, %d images, %d audios",
            ...
            err = str(e).lower()
            logger.warning("LikeeExtractor DownloadError: %s", e)
            if "private" in err or "unavailable" in err:
                return {
                    "status": "error",
                    "code": "LIKEE_PRIVATE_CONTENT",
                    "context": str(e),
                    "data": None,
                }
            return {
                "status": "error",
                "code": "LIKEE_EXTRACT_ERROR",
                "context": str(e),
                "data": None,
            }
        except Exception as e:
            logger.exception("LikeeExtractor Exception: %s", e)
            return {
                "status": "error",
                "code": "LIKEE_UNEXPECTED_ERROR",
                "context": str(e),
                "data": None,
            }
